...
- H2.3 adds source-bound `VisualEvidenceFixture` observations for `VISUAL_STATE_CHANGE` and `OCR_STATE_CHANGE`. These observations may carry an explicit factual hypothesis and small provenance metadata, but they remain retrieval evidence rather than creator/editorial truth; benchmark GT must not be replayed as if it were detector output;
- manual markers, transcripts and visual/OCR evidence fixtures are bound to exact source SHA-256 + duration, carry no creator/editorial score, and can enrich both `hybrid proposals` and the provider-free `hybrid run-fixture` path;
