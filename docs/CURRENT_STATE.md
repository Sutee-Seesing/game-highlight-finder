...
Hybrid implementation is now coherent through H6 plus the first H2.1 proposal-enrichment slice, all provider-free. H1 adds `StoryState`, `ResolutionState`, evidence-backed `CandidateClaim`/`ClaimStatus`, and a ranking/report gate that prevents provisional/unverified `STANDALONE_STORY` items from entering normal creator review. H2 adds provider-neutral factual `Proposal` artifacts and a deliberately weak local activity adapter with no creator/editorial score. H2.1 adds source-bound `ManualProposalMarkerSet` fixtures and deterministic clustering of nearby compatible factual anchors; conflicting explicit event hypotheses are never merged, and source provenance/signal types remain visible. H3 adds bounded proposal-centered context planning and dynamic expansion. H4 adds a fake semantic judge whose outcome claims remain provisional. H5 adds an independent `CandidateVerification` / fake resolution verifier; verified or contradicted claims require timestamped evidence. H6 adds verified `StoryAssembly` boundaries for setup/event/payoff/reaction and refuses story assembly before `COMPLETE` + `VERIFIED/NOT_APPLICABLE`. `run_provider_free_hybrid_triage` joins those contracts end-to-end and persists full semantic/review maps under `session/hybrid/`; clip boundaries are derived only after the creator-review verification gate. Local CLI/session workflow now includes `hybrid proposals`, `hybrid run-fixture`, `hybrid review-template`, and `hybrid review-summary`; optional manual markers can enrich both proposal preparation and fixture runs without provider I/O. A separate creator-evaluation corpus records KEEP/MAYBE/rejection, owner role/boundary corrections, review time and `MISS_OBVIOUS` without contaminating GT. Latest focused Hybrid/CLI/M6 regression is **40 passed in 56.44s**, Ruff is green, mypy reports no issues in **72 source files**, and `git diff --check` is clean apart from line-ending normalization warnings. No provider calls/uploads were made by this work.
