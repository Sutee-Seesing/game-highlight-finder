...
The next provider-free continuation is narrowed to the single unresolved MUST_CATCH proposal `574-594s`. Under the no-SDK-retry identity it requires **1 planned generation call / THB 0.385248 maximum reservation**, with **0 provider calls / 0 uploads / 0 ledger reservations / 0 judge cache hits** during preflight. Its helper is syntax-valid, uses a proposed **attempt cap 1 / exposure cap THB 0.39 / SDK HTTP attempts 1 / no application retry**, and its dedicated ledger/summary are absent. A fresh explicit authorization is required before any live continuation.
