...
The first authorized live Round A dispatch on 2026-09-03 consumed **7 logical router requests** because every model failed at the MUST_CATCH request and therefore triggered the locked early-stop before WORTH_REVIEW. This was **not a semantic model failure**: all seven requests returned OpenRouter HTTP 404 `No endpoints found that satisfy the max price for this request`, with no completed model response and **THB 0 settled**. Root cause was a transport-unit defect: `provider.max_price` was sent in USD per token even though OpenRouter routing expects USD per million tokens, making every ceiling 1,000,000x too low and filtering all upstream endpoints. The pre-remediation ledger conservatively retains seven AMBIGUOUS rows / **THB 2.025090 unresolved reservation** because the exact error routing metadata was not persisted. Remediation now sends the documented USD-per-million ceiling and interprets explicit router `attempt=0` as confirmed pre-upstream rejection, allowing the reservation to be released instead of becoming ambiguous; missing routing evidence remains conservative. The replacement request identity is advanced to `hybrid-judge-openrouter-v3` and fingerprints `routing_price_unit=usd_per_million_tokens`, so the corrected request cannot collide with the incident's v2 call IDs/artifacts. Provider-free regression after the v3 identity hardening is **49/49 targeted PASS**, Ruff PASS over `src + tests`, mypy PASS over **79 source files**, and canonical full pytest **395/395 PASS in 100.27s**. A fresh provider-free v3 preflight again quotes **14 planned calls / THB 4.663985 maximum reservation**, with provider calls/uploads/reservations/cache hits all zero, **0 incident call-ID collisions**, and only an empty initialized dry ledger (`calls=0`, `ledger_events=0`, `ledger_control=0`). No automatic retry was made and this incident does not authorize a replacement paid round; any new live dispatch requires a fresh explicit attempt/exposure boundary and fresh ledger identity.
