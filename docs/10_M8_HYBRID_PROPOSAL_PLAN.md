...
The authorized v4 recovery completed in **9 calls** with **THB 1.267537 settled + THB 1.349723 unresolved = THB 2.617260 known-plus-unresolved**, safely below the THB 3.15 hard cap. A provider-free re-audit against the canonical `m8-eval-v1` ruler found one benchmark-helper defect: the recovery script's `caught` flag used any temporal overlap rather than the canonical IoU/boundary policy. Under the real evaluator, Qwen3.8-Flash does **not** qualify on MUST_CATCH (best IoU 0.125), Qwen3.8-Max does **not** qualify (best IoU 0.1875), and GLM-5.3-Flash remains a miss. Seed-2.0-Mini **does** qualify on MUST_CATCH (574-579.5s, IoU 0.34375) but its WORTH_REVIEW call exhausted the completion budget. Seed Mini also reported **1,906 thinking tokens** despite `reasoning.max_tokens=1,024`, so the upstream sub-cap is non-authoritative while the 2,048 combined completion/cost ceiling remains authoritative. **Seed-2.0-Lite still passes the full Round A semantic gate under the canonical evaluator**, with MUST_CATCH 574-580s (IoU 0.375) and WORTH_REVIEW 401-405s (IoU 0.25), both settled `finish_reason=stop`, router attempt=1/provider=Seed. Its two calls settled at **THB 0.215223 total**.

Round B is now paused before any paid execution because the calibration truth is not exhaustive enough for a valid precision claim. Data-identity audit is clean: both source SHA-256 values match dataset/annotation metadata, both annotation SHA-256 values match their locked provenance, and the 21 ms source/proxy offset is correctly compensated. But provenance says `human_full_video_review=false` and audio transcription is incomplete. Of the **13 proposal clips**, **5 overlap known highlights and 8 remain without locked positive/negative adjudication**. Counting Seed Lite detections in those 8 clips as false positives would be methodologically wrong. The previous 11-new-call preflight (**THB 2.492524 maximum new reservation**) is retained only as a cost estimate; its runner must not be launched yet. Complete and lock human proposal-level adjudication for those 8 clips first, then regenerate the Round B evaluation/preflight so precision/value is measured against real exhaustive proposal-level truth. The two already settled Seed Lite Round A positives should still be reused rather than rerolled.

The provider-free completion workflow is prepared and active. Eight unlabeled proposal clips are exposed through the existing local review-queue UI using hard links to the committed proposal media; queue set `h5-round-b-unadjudicated-8-v1` contains no Seed Lite/Qwen/other provider predictions. The loopback server is task `df2f9d6b-2885-49e5-b23d-4ef88a89c449`, URL `127.0.0.1:61414`, provider calls zero. A local lock compiler requires queue-hash-bound `HUMAN` adjudication with exactly 8 resolved decisions and **no `UNCERTAIN`**, then combines them with the 5 known annotation-overlap positives into exactly 13 proposal-level labels. This lock is explicitly `ROUND_B_PROPOSAL_LEVEL_PRECISION_VALUE_ONLY`; it does not alter the temporal annotation files. The Round B paid runner now verifies the lock, reviewer kind, count, provider-free provenance, and current queue/adjudication SHA-256 values before it can reach API-key/provider setup. Both compiler and runner have been proven to fail closed in the present no-sidecar/no-lock state.
