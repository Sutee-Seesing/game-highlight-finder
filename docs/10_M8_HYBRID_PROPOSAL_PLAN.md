...
Provider-free remediation now pins only the Hybrid Judge transport to the stable Gemini Interactions API **`v1`**. The API version is included in the provider-request/cost fingerprint and persisted request metadata, and the Hybrid Judge fails closed before reservation/upload/generation unless the injected transport explicitly declares `api_version="v1"`; this prevents a fingerprint/endpoint mismatch with the SDK default version. Verification is **35/35 targeted PASS, Ruff PASS, mypy PASS over 76 source files, and 367/367 full pytest PASS in 111.27 s**. Fresh stable-v1 preflight makes **0 provider calls / 0 uploads / 0 ledger reservations**, has **0 Hybrid Judge cache hits** under the changed request identity, and retains the cal-02 quote of **6 calls / THB 2.376752 maximum reservation**. The dedicated stable-v1 live ledger and summary are still absent, so **no stable-v1 live call has been made**. Because this is a new request/endpoint identity, live continuation requires fresh explicit authorization rather than consuming the previous cal-02 approval.

A narrower provider-free continuation preflight now selects only the two unresolved cal-02 proposals that cover the known positives: `386-421s` overlaps `hl-0001` (`401-417s`) and `574-594s` overlaps MUST_CATCH `hl-0002` (`574-590s`). The four prior settled REJECTs are outside both annotated positives, so rerunning them is unnecessary for the remaining H5 semantic question. The stable-v1 subset requires **2 planned generation calls / THB 0.816844 maximum reservation**, with **0 provider calls / 0 uploads / 0 ledger reservations** during preflight and 0 judge cache hits under the stable-v1 identity. A dedicated helper is syntax-valid and hard-guards **attempt cap 2 / exposure cap THB 0.82 / no automatic retry / API v1**; its live ledger and summary are absent. This two-call subset is the preferred next live experiment if separately authorized.
