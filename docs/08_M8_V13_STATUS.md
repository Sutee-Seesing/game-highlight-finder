...
## 2026-09-01 deterministic control-window outcome

The provider-free follow-up sampled eight deterministic non-overlapping FreeDoom/Xonotic control windows and reviewed them visually without consulting audio values. The combined private queue now contains **20** `ASSISTANT_VISUAL` decisions: the original 12 high-audio windows remain POSITIVE, while the controls resolve to **3 POSITIVE / 5 BORING** (FreeDoom controls: 1 positive, 3 boring; Xonotic controls: 2 positive, 2 boring). The combined sidecar is queue-SHA-bound (`8d626eeb...55594`), stays separate from human ground truth, is excluded from M8 acceptance, and records `provider_calls=0`.

Running `benchmark cross-case-suppression` against that exact 20-window queue now succeeds as a diagnostic and produces `NORMALIZED_AUDIO_PEAK_NO_CLEAN_SEPARATION`. Protecting all 15 visual positives sets the normalized `audio_peak_over_loudness_db` floor to **3.140291 dB**, but **0/5** BORING controls fall below it; all five survive. This falsifies the current hypothesis that normalized peak-over-source-loudness alone can provide a safe monotonic negative-suppression rule across these sources. `production_threshold_locked` remains **false**. The next provider-free calibration work must therefore use independent already-available local signals or combinations rather than further tuning this one-dimensional audio feature. Revealed v13 validation remains forbidden for tuning, M8 remains **NOT ACCEPTED**, M9 remains **NOT STARTED**, and any fresh Gemini call still requires new explicit authorization.


## 2026-09-01 control audio-domain correction

A provenance check immediately after the first control evaluation found that the eight new control peaks had been measured with `astats` directly on the source WebM, while the original 12 cross-case peaks and each source overall-loudness baseline came from the runtime `local-signals/activity.json` domain. Subtracting the local-signals LUFS baseline from source-domain peaks mixed two signal scales, so the previously recorded **0/5** rejection count is superseded and must not be used. No semantic label changed.

The control audio features were rebuilt from the same persisted local-signals bins used by the original cross-case intervals, then the 20-decision assistant sidecar was rebound to the new queue SHA (`abb0004d...098e9`). The corrected provider-free evaluator still returns `NORMALIZED_AUDIO_PEAK_NO_CLEAN_SEPARATION`, but the quantitative result is now **3/5 BORING controls rejected** while all **15/15 POSITIVE** windows remain protected at the same **3.140291 dB** normalized-positive floor. The surviving boring controls are `freedoom-control-03` and `xonotic-control-03`. Thus normalized peak prominence has useful partial rejection but is not a safe standalone separator. `production_threshold_locked=false`, M8 remains **NOT ACCEPTED**, revealed v13 remains forbidden for tuning, and provider/API calls remain **ZERO**.


## 2026-09-01 provider-free v19 semantic prompt checkpoint

A provider-free proxy-motion exploration (`signalstats.YDIF` sampled at 4 fps) did not produce a clean cross-source separator for the two boring controls that survive corrected normalized-audio suppression. Rather than add an overfit motion threshold, experimental `gemini-scout-window-v19` now hardens Scout semantics while leaving v18 behavior and the configured default unchanged. v19 treats local-signal hints as navigation cues only, rejects pure traversal/rotation/idle weapon movement/ambient or UI-only activity when no visible event or payoff exists, requires candidate evidence to cite visible on-screen cues, and explicitly preserves visibly real but uncertain interactions at lower score/confidence to protect recall.

The checkpoint is fully provider-free and regression-green: targeted Gemini tests pass **26/26 in 30.21s**, Ruff passes `src` + `tests`, mypy passes **73 source files**, and the full suite passes **335/335 in 440.29s** (task `d018f96b-9560-49f9-a3b0-b17e6901a580`). No provider/API call was made. v19 remains **non-default** and has no live quality claim yet; any live calibration call requires fresh explicit authorization. Revealed v13 validation remains forbidden for tuning, `production_threshold_locked=false`, M8 remains **NOT ACCEPTED**, and future unbiased acceptance still requires a fresh locked holdout.
