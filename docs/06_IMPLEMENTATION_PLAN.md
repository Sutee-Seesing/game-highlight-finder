...
Provider-free v4 remediation now aligns the wire contract with the reservation: with reasoning enabled it sends a combined completion ceiling of **2,048 tokens** plus `reasoning.max_tokens=1,024`, while the cost quote remains unchanged at 1,024 final + 1,024 thinking. Successful HTTP responses can preserve authoritative usage even when final text is empty; the pipeline settles cost and writes the raw envelope before locally rejecting `finish_reason=length` or missing final text, preventing those successful-provider cases from becoming AMBIGUOUS solely because semantic parsing cannot proceed. Request identity is advanced to `hybrid-judge-openrouter-v4` and media transport contract `openrouter-base64-video-v2`. Final provider-free verification is **51/51 targeted PASS**, Ruff PASS over `src + tests`, mypy PASS over **79 source files**, and canonical full pytest **397/397 PASS in 98.23s** under durable task `c60cc148-6443-4275-a8fb-427f007b1ea5`. A full seven-model v4 preflight remains **14 planned calls / THB 4.663985 maximum reservation** with zero provider I/O, reservations, cache hits, or prior-call collisions, but it is no longer the recommended recovery scope: MiMo-V2.5 and GLM-5V-Turbo already produced complete settled `finish_reason=stop` MUST_CATCH responses and definitive semantic `REJECT` decisions, so rerunning them would be a stochastic reroll rather than transport recovery. The operational v4 recovery therefore contains only the five transport-inconclusive models (Qwen3.8-Flash, GLM-5.3-Flash, Seed-2.0-Mini, Seed-2.0-Lite, Qwen3.8-Max). Its provider-free preflight is **10 planned calls / THB 3.147282 maximum reservation**, with provider calls/uploads/reservations/cache hits all zero, zero prior-call collisions, and zero persisted call/event/control rows.

The explicitly authorized five-model v4 recovery completed under task `7188ad28-b31b-4c44-a79b-5c4dd0836600` with **9 logical calls**, HTTP attempts=1, no automatic retry, **THB 1.267537 settled**, **THB 1.349723 unresolved reservation**, and **THB 2.617260 known-plus-unresolved exposure**, below the THB 3.15 hard cap. A provider-free post-run audit then found that the local recovery helper had used a looser `any temporal overlap` rule for its `caught` flag instead of the canonical `m8-eval-v1` ruler (`IoU >= 0.25`, or the documented paired-boundary tolerance rule). Re-adjudication with the canonical evaluator changes two intermediate labels: Qwen3.8-Flash's 589-593s / 577-579s events have IoU 0.0526 / 0.125 and **do not** qualify as MUST_CATCH; Qwen3.8-Max's 576.5-579.5s / 579.5-581s events have IoU 0.1875 / 0.09375 and also **do not** qualify. Their later WORTH_REVIEW HTTP 413 calls are therefore extraneous evidence and do not make them Round A contenders. GLM-5.3-Flash remains a strict MUST_CATCH miss at 591.5-594s. Seed-2.0-Mini **does** qualify on MUST_CATCH (574-579.5s, IoU 0.34375) but its WORTH_REVIEW response hit `finish_reason=length`. **Seed-2.0-Lite remains the only Round A winner even under the canonical evaluator:** MUST_CATCH 574-580s has IoU 0.375 and WORTH_REVIEW 401-405s has IoU exactly 0.25; both calls are settled `finish_reason=stop`, router attempt=1/provider=Seed. Its two settled calls cost **THB 0.215223** total. Seed Mini also exposed a provider quirk: on WORTH_REVIEW it reported 1,906 thinking tokens despite the requested `reasoning.max_tokens=1,024`, consuming the full 2,048 combined ceiling; the total completion/cost cap still held, but the sub-cap is not reliable for this endpoint.

The same audit also blocks the previously proposed paid Round B precision/value run. Source SHA-256 values match the dataset and annotation documents exactly, annotation SHA-256 values match their locked provenance exactly, and the 21 ms source/proxy timestamp origin is correctly compensated by `source_to_proxy_ms`, so there is no evidence of source/session corruption or timeline drift. However, the calibration provenance explicitly records `human_full_video_review=false` and incomplete audio transcription. Across the 13 proposal clips, **5 overlap currently known highlights and 8 have no locked positive/negative adjudication**. Treating every Seed Lite output in those 8 clips as a false positive would therefore fabricate precision from incomplete ground truth, contradicting the benchmark protocol. The provider-free 11-new-call preflight (**THB 2.492524 maximum reservation**) remains useful as a cost quote only; **no Round B paid execution should occur yet**. First complete and lock proposal-level human adjudication for the 8 unlabeled clips (or otherwise create exhaustive calibration truth) before model predictions. Only then may the 2 settled Round A responses be reused and the remaining paid scope be re-preflighted against the locked labels.

A provider-free human-review pack is now prepared for exactly those eight unlabeled proposals. It uses hard links to the already-generated proposal clips (no media duplication), queue set `h5-round-b-unadjudicated-8-v1`, and the existing loopback `benchmark review-queue` UI; model predictions are not included in the queue. The UI is running under durable task `df2f9d6b-2885-49e5-b23d-4ef88a89c449` at `127.0.0.1:61414` with provider calls **ZERO**. A local ground-truth-lock compiler is also prepared: it refuses to lock unless the sidecar is queue-SHA-bound, `reviewer_kind=HUMAN`, covers all 8 review IDs, contains no `UNCERTAIN`, and combines those decisions with the 5 pre-locked annotation-overlap positives into exactly 13 unique proposal-level labels. The resulting lock is explicitly scoped to Round B proposal-level precision/value and cannot replace temporal BenchmarkAnnotations or final M8 holdout acceptance. The Seed Lite Round B runner independently refuses execution unless that 13-proposal lock exists and its queue/adjudication hashes still match. Compile checks pass, the compiler currently fails closed because no human sidecar exists yet, and the paid runner likewise fails closed before API-key/provider access because the lock is absent.
