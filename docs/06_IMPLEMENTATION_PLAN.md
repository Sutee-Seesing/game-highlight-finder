...
The explicitly authorized five-model v4 recovery completed under task `7188ad28-b31b-4c44-a79b-5c4dd0836600` with **9 logical calls**, HTTP attempts=1, no automatic retry, **THB 1.267537 settled**, **THB 1.349723 unresolved reservation**, and **THB 2.617260 known-plus-unresolved exposure**, below the THB 3.15 hard cap. Qwen3.8-Flash caught MUST_CATCH but its WORTH_REVIEW request returned HTTP 413 and remains AMBIGUOUS; GLM-5.3-Flash returned `KEEP` on MUST_CATCH but its only event started at 591.5s, outside the locked 574-590s annotation, so it strict-missed and WORTH_REVIEW was skipped; Seed-2.0-Mini caught MUST_CATCH but its WORTH_REVIEW response hit `finish_reason=length`; Qwen3.8-Max caught MUST_CATCH but its WORTH_REVIEW request also returned HTTP 413 and remains AMBIGUOUS. **Seed-2.0-Lite is the only Round A winner:** it returned settled `finish_reason=stop` responses with router attempt=1/provider=Seed for both positives, catching MUST_CATCH at 574-580s and WORTH_REVIEW at 401-405s. Its two settled calls cost **THB 0.215223** total. Seed Mini also exposed a provider quirk: on WORTH_REVIEW it reported 1,906 thinking tokens despite the requested `reasoning.max_tokens=1,024`, consuming the full 2,048 combined ceiling; the total completion/cost cap still held, but the sub-cap is not reliable for this endpoint. No paid retry is authorized for the 413 or truncation cases. Seed-2.0-Lite alone advances to the provider-free Round B precision/value preflight over the locked 13-proposal / 302-second calibration set. A full fresh 13-call preflight quotes THB 2.971575, but rerunning the two settled Round A positives would waste money and introduce stochastic reroll noise. The locked Round B execution plan therefore reuses those two settled Seed Lite v4 responses and sends only the remaining **11 new proposals / 247 seconds**. Provider-free preflight for that exact execution scope is **11 planned calls / THB 2.492524 maximum new reservation**, with zero provider calls/uploads/reservations. A dedicated fresh runner/secure launcher compiles successfully, enforces attempt cap 11, HTTP attempts=1, no automatic retry, and a proposed THB 2.50 hard exposure cap; its live ledger/preflight-ledger/summary artifacts are absent. Round B paid execution is not yet authorized.
