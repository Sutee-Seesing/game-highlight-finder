...
13. Next provider-free step: implement the real semantic-judge/verifier request contract plus exact cost preflight over routed `analysis_context.mp4` clips, including logical-call counting, media-minute accounting, cache identity, no-auto-retry policy, and a hard block unless a fresh explicit THB authorization exists. No provider generation or upload is authorized by normal chat continuation.
14. After the provider boundary/preflight contract is coherent, add or exercise one richer non-audio factual evidence source on a non-FPS/social source (transcript first where a local transcript fixture is available; otherwise a bounded visual/OCR fixture) and measure whether it raises proposal recall without exploding selected semantic-inspection density.
