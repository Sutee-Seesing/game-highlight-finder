"""Ingest stage state transitions."""
...
def recover_interrupted(manifest: Manifest, *, now: datetime | None = None) -> bool:
    ...
    error = ErrorRecord(
        category="internal",
        message="Previous ingest attempt was interrupted before completion.",
        retryable=True,
    )
...
def start_ingest(
    ...
    if stage.status not in {StageStatus.PENDING, StageStatus.FAILED, StageStatus.STALE}:
        raise ValidationError(f"Cannot start ingest from state {stage.status}.")
    timestamp = now or utc_now()
...
def complete_ingest(
    ...
    *,
    ...
    stage = manifest.stages["ingest"]
    if stage.status is not StageStatus.RUNNING or not stage.attempts:
        raise ValidationError("Cannot complete ingest when it is not running.")
    timestamp = now or utc_now()
...
def fail_ingest(manifest: Manifest, error: ErrorRecord, *, now: datetime | None = None) -> None:
    stage = manifest.stages["ingest"]
    if stage.status is not StageStatus.RUNNING or not stage.attempts:
        raise ValidationError("Cannot fail ingest when it is not running.")
    timestamp = now or utc_now()
    attempt = stage.attempts[-1]
