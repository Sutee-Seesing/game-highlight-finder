...
def test_lock_verification_and_calibration_plan_exclude_validation(tmp_path: Path) -> None:
    ...
    assert CALIBRATION_EXPERIMENT_REVISION == "v11"
    assert all(arm.result_set_id.endswith("-v11") for arm in plan.arms)
    ...
    assert plan.arms[1].effective_media_config["estimated_video_tokens_per_second"] == 70
    assert (
